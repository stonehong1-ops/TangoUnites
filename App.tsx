...
const AppContent: React.FC = () => {
    ...
    return (
        <div className={`${(mainTab === 'feed' || mainTab === 'content') ? 'bg-white' : 'bg-gray-100'} text-gray-900 pb-20 relative overflow-hidden`}>
            ...
            {selectedUserContext && <UserProfileModal user={selectedUserContext.user} contextGroup={selectedUserContext.group} onClose={() => setSelectedUserContext(null)} onEdit={(u) => { setSelectedUserContext(null); setUserToEdit(u); setAddUserModalOpen(true); }} currentUser={currentUser} onSetStaff={handleSetStaff} onStartConversation={handleStartConversation} />}
            ...
            {isAddUserModalOpen && <AddUserModal onClose={() => { setAddUserModalOpen(false); setUserToEdit(null); }} onSaveUser={handleSaveUser} userToEdit={userToEdit} onDeleteUser={(id) => setUsers(prev => prev.filter(u => u.id !== id))} language={language} setLanguage={setLanguage} languages={languages} />}
